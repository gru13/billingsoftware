...
class bill_menu:
    def __init__(self, file_data):
        ...
        self.close_win = Button(self.Win_bill_menu, text="close", command=lambda: self.Win_bill_menu.destroy())
        self.file_data = file_data
        self.menu = menu_bar(self.Win_bill_menu, self.file_data, bill_menu)
        self.billed_saree = []
        now = datetime.now()
        bill_no = now.strftime(r"%d%m%y")
        self.bill_no = bill_no
        self.today = now.strftime(r"%d-%m-%y")
        ...
        self.SAREE_NO_FROM_DATABASE = [str(a.value) for a in self.WHOLE_DATA["A"]]
        ...
        self.total = 0
        Label(self.Win_bill_menu, text="ABIRAMI SILKS", font=("ARIAL", 45)).grid(column=0, row=0, columnspan=10)
        Label(self.Win_bill_menu, text="Name", font=("ARIAL", 18)).grid(column=0, row=1)
        Label(self.Win_bill_menu, text="Number", font=("ARIAL", 18)).grid(column=2, row=1)
        Label(self.Win_bill_menu, text="Address", font=("ARIAL", 18)).grid(column=0, row=2)
        ...
        self.name_entry = Entry(self.Win_bill_menu, justify=CENTER, bd=2, font=("ARIAL", 18))
        self.name_entry.grid(column=1, row=1)

        self.number_entry = Entry(self.Win_bill_menu, justify=CENTER, bd=2, font=("ARIAL", 18))
        self.number_entry.grid(column=3, row=1)

        self.address_entry = Entry(self.Win_bill_menu, justify=CENTER, bd=2, width=50, font=("ARIAL", 18))
        self.address_entry.grid(column=1, row=2, columnspan=5)

        self.saree_num_entry = Entry(self.Win_bill_menu, justify=CENTER, bd=3, font=("ARIAL", 15))
        self.saree_num_entry.grid(column=1, row=3)
        self.saree_num_entry.bind("<Return>", self.add_saree_to_bill)

        Label(self.Win_bill_menu, text="SNO", width=10, font=("ARIAL", 11, "bold")).grid(row=4, column=0)
        Label(self.Win_bill_menu, text="SAREE NO", width=10, font=("ARIAL", 11, "bold")).grid(row=4, column=1)
        Label(self.Win_bill_menu, text="DESIGN", width=10, font=("ARIAL", 11, "bold")).grid(row=4, column=2)
        Label(self.Win_bill_menu, text="COLOUR", width=10, font=("ARIAL", 11, "bold")).grid(row=4, column=3)
        Label(self.Win_bill_menu, text="PRICE", width=10, font=("ARIAL", 11, "bold")).grid(row=4, column=4)

        self.total_label = Label(self.Win_bill_menu, text=f"TOTAL={str(self.total)}", font=("ARIAL", 11))

        self.bill_button = Button(text="BILL", font=("comforta", 11, "bold"), command=self.bill)
    ...
    def add_saree_to_bill(self, event):
        ...
        for saree_no in saree_nos:
            if saree_no in self.UNSOLD_SAREE_NO_FROM_DATABASE and saree_no != "":
                ...
                price = Entry(self.Win_bill_menu, font=("ARIAL", 11), justify=CENTER)
                price.grid(column=4, row=int(sno) + 4)
                saree_detail = [sno, saree_no, design, colour, str(row_no), price]
                ...
                if self.WHOLE_DATA["E" + str(row_no)].value is None:
                    ...
                    Label(self.Win_bill_menu, text=colour, width=10, font=("ARIAL", 11)).grid(column=3,
                                                                                              row=4 + int(sno))
                Label(self.Win_bill_menu, text=sno, width=10, font=("ARIAL", 11)).grid(column=0, row=4 + int(sno))
                Label(self.Win_bill_menu, text=saree_no, width=10, font=("ARIAL", 11)).grid(column=1, row=4 + int(sno))
                Label(self.Win_bill_menu, text=design, width=10, font=("ARIAL", 11)).grid(column=2, row=4 + int(sno))
    ...
    def bill(self):
        ...
        for saree in self.billed_saree:
            ...
            if type(price) != (int or float):
                ...
                # billed_saree structure [sno, saree_no, design, colour,str(row_no), price]
                showerror(title="Enter the price", message=f"you  did not give the price of saree(saree no:{saree[1]})")
            saree[-1] = price
        ...
        for one_saree in self.billed_saree:
            ...
            self.WHOLE_DATA["E" + one_saree[-2]] = one_saree[3]  # customer name
            self.WHOLE_DATA["G" + one_saree[-2]] = self.today  # d.o.s
            self.WHOLE_DATA["H" + one_saree[-2]] = one_saree[-1]  # sold price
            self.WHOLE_DATA["I" + one_saree[-2]] = self.name_entry.get()  # customer name
            self.WHOLE_DATA["J" + one_saree[-2]] = self.number_entry.get()  # customer number
            self.WHOLE_DATA["K" + one_saree[-2]] = self.address_entry.get()  # customer address
